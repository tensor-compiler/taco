...
// TODO (rohany): Move this to a utility file.
...
// Temporary method to just generate an hdf5 file.
...
void top_level_task(const Task* task, const std::vector<PhysicalRegion> &regions, Context ctx, Runtime* runtime) {
  ...
  if (dumpOnly) {
    ...
    // TODO (rohany): Extract this logic into separate helper method for HDF5 files that
    //  are generated by this tool.
    auto hdf5file = H5Fopen(hdf5Filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    H5G_info_t info;
    assert(H5Gget_info(hdf5file, &info) >= 0);
    auto order = info.nlinks - 1;
    // Assume that there's always going to be a "value" column.
    auto dset = H5Dopen1(hdf5file, "value");
    assert(dset >= 0);
