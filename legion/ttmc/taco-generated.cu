...
void computeLegion(Context ctx, Runtime* runtime, LogicalRegion A, LogicalRegion B, LogicalRegion C, int32_t pieces) {
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    int32_t in = (*itr)[0];
    Point<3> AStart = Point<3>((in * ((B1_dimension + (pieces - 1)) / pieces) + 0 / pieces), 0, 0);
    Point<3> AEnd = Point<3>(TACO_MIN((in * ((B1_dimension + (pieces - 1)) / pieces) + ((B1_dimension + (pieces - 1)) / pieces - 1)),ADomain.hi()[0]), TACO_MIN(B2_dimension,ADomain.hi()[1]), TACO_MIN(C2_dimension,ADomain.hi()[2]));
    Rect<3> ARect = Rect<3>(AStart, AEnd);
    if (!ADomain.contains(ARect.lo) || !ADomain.contains(ARect.hi)) {
      ARect = ARect.make_empty();
    }
    AColoring[(*itr)] = ARect;
    Point<3> BStart = Point<3>((in * ((B1_dimension + (pieces - 1)) / pieces) + 0 / pieces), 0, 0);
    Point<3> BEnd = Point<3>(TACO_MIN((in * ((B1_dimension + (pieces - 1)) / pieces) + ((B1_dimension + (pieces - 1)) / pieces - 1)),BDomain.hi()[0]), TACO_MIN(B2_dimension,BDomain.hi()[1]), TACO_MIN(C1_dimension,BDomain.hi()[2]));
    Rect<3> BRect = Rect<3>(BStart, BEnd);
    if (!BDomain.contains(BRect.lo) || !BDomain.contains(BRect.hi)) {
      BRect = BRect.make_empty();
    }
