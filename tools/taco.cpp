...
int main(int argc, char* argv[]) {
  ...
  if (!evaluate) {
    if (time) {
      TACO_BENCHMARK(tensor.compile(),1,timevalue);
      std::cout << "Compile Time " << timevalue;
    }
    else {
      tensor.compile();
    }
  ...
  if (evaluate) {
    ...
    for ( const auto &fills : tensorsFill ) {
      ...
      taco::util::fillTensor(paramTensor, fills.second, 6);
      std::cout << "filling "<< paramTensor << std::endl;
    }
    tensor.compile();
    tensor.assemble();
    tensor.compute();
