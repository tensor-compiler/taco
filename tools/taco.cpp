...
// TODO: probably should move into different file?
static TensorBase getCOOTensor(TensorBase tensor) {
  const string tensorName = tensor.getName();
  const Datatype ctype = tensor.getComponentType();
  const vector<int> dimensions = tensor.getDimensions();
  const Format format = tensor.getFormat();

  const Format bufferFormat = COO(format.getOrder(), false, true, false, 
                                  format.getModeOrdering());

  return TensorBase(tensorName + "_COO", ctype, dimensions, bufferFormat);
}
...
int main(int argc, char* argv[]) {
  ...
  string packComment = "// Converts coordinate and value arrays in COO format to ";
  vector<pair<ir::Stmt, string>> packs; 
  for (auto a : getArgumentAccesses(stmt)) {
    const string tensorName = a.getTensorVar().getName();
    ...
    IndexStmt packStmt = (tensor(indexVars) = bufferTensor(indexVars));
    ...
    stringstream ss; 
    ss << packComment << format;

    packs.push_back(make_pair(
        lower(packStmt, "pack_" + tensorName, true, true, true), 
        ss.str()));
  }
  ...
  stringstream ss; 
  ss << resultFormat; 
  string unpackComment = "// Converts " + ss.str() + " to coordinate and value arrays in COO format";
  ...
  if (printKernels) {
    ...
    for (auto pack : packs) {
      cout << endl << pack.second;
      codegen->compile(pack.first, false);
      cout << endl << endl;
    }

    if (unpack.defined() ) {
      cout << endl << unpackComment;
  ...
  if (writeKernels) {
    ...
    for (auto pack : packs) {
      filestream << endl << pack.second;
      codegenFile->compile(pack.first, !hasPrinted);
      hasPrinted = true;
    }
    if (unpack.defined() ) {
      filestream << endl << unpackComment;
