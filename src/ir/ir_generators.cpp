...
Stmt compoundStore(Expr a, Expr i, Expr val, bool use_atomics, PARALLEL_UNIT atomic_parallel_unit) {
...
Stmt compoundAssign(Expr a, Expr val, bool use_atomics, PARALLEL_UNIT atomic_parallel_unit) {
  Expr add = (val.type().getKind() == Datatype::Bool) 
