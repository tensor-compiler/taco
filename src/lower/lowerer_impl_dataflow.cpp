...
class LowererImplDataflow::Visitor : public IndexNotationVisitorStrict {
public:
  Visitor(LowererImplDataflow* impl) : impl(impl) {}
...
LowererImplDataflow::LowererImplDataflow() : visitor(new Visitor(this)) {
}
...
Stmt
LowererImplDataflow::lower(IndexStmt stmt, string name,
...
Stmt LowererImplDataflow::lowerYield(Yield yield) {
  std::vector<Expr> coords;
...
LowererImplDataflow::splitAppenderAndInserters(const vector<Iterator>& results) {
...
Stmt LowererImplDataflow::lowerForall(Forall forall)
{
...
Stmt LowererImplDataflow::lowerForallCloned(Forall forall) {
...
Stmt LowererImplDataflow::searchForFusedPositionStart(Forall forall, Iterator posIterator) {
                                       ...
                                       ir::Stmt recoveryStmt)
{
  ...
  Stmt LowererImplDataflow::lowerForallDenseAcceleration(Forall forall,
                                                 vector<Iterator> locators,
                                                 vector<Iterator> inserters,
                                                 vector<Iterator> appenders,
                                                 set<Access> reducedAccesses,
...
Stmt LowererImplDataflow::lowerForallCoordinate(Forall forall, Iterator iterator,
                                        vector<Iterator> locators,
                                        vector<Iterator> inserters,
                                        vector<Iterator> appenders,
                                        set<Access> reducedAccesses,
...
Stmt LowererImplDataflow::lowerForallPosition(Forall forall, Iterator iterator,
                                      vector<Iterator> locators,
                                      vector<Iterator> inserters,
                                      vector<Iterator> appenders,
                                      set<Access> reducedAccesses,
...
Stmt LowererImplDataflow::lowerForallFusedPosition(Forall forall, Iterator iterator,
                                      vector<Iterator> locators,
                                      vector<Iterator> inserters,
                                      vector<Iterator> appenders,
                                      set<Access> reducedAccesses,
...
Stmt LowererImplDataflow::lowerMergeLattice(MergeLattice lattice, IndexVar coordinateVar,
                                    IndexStmt statement,
...
Stmt LowererImplDataflow::lowerMergePoint(MergeLattice pointLattice,
...
Stmt LowererImplDataflow::resolveCoordinate(std::vector<Iterator> mergers, ir::Expr coordinate, bool emitVarDecl) {
  if (mergers.size() == 1) {
...
Stmt LowererImplDataflow::lowerMergeCases(ir::Expr coordinate, IndexVar coordinateVar, IndexStmt stmt,
...
Stmt LowererImplDataflow::lowerForallBody(Expr coordinate, IndexStmt stmt,
                                  vector<Iterator> locators,
                                  vector<Iterator> inserters,
                                  vector<Iterator> appenders,
...
vector<Stmt> LowererImplDataflow::codeToInitializeDenseAcceleratorArrays(Where where) {
  TensorVar temporary = where.getTemporary();
...
//       the generated code.
std::pair<bool,bool> LowererImplDataflow::canAccelerateDenseTemp(Where where) {
...
Stmt LowererImplDataflow::lowerForallReductionBody(Expr coordinate, IndexStmt stmt,
                                  vector<Iterator> locators,
                                  vector<Iterator> inserters,
                                  vector<Iterator> appenders,
                                  const set<Access>& reducedAccesses) {
...
vector<tuple<Stmt, Expr>> LowererImplDataflow::lowerForallBulk(Forall forall, Expr coordinate, IndexStmt stmt,
                                           vector<Iterator> locators,
                                           vector<Iterator> inserters,
                                           vector<Iterator> appenders,
...
vector<Stmt> LowererImplDataflow::codeToInitializeTemporary(Where where) {
  TensorVar temporary = where.getTemporary();
...
Stmt LowererImplDataflow::lowerWhere(Where where) {
  TensorVar temporary = where.getTemporary();
...
Stmt LowererImplDataflow::lowerSequence(Sequence sequence) {
...
Stmt LowererImplDataflow::lowerAssemble(Assemble assemble) {
...
Stmt LowererImplDataflow::lowerMulti(Multi multi) {
  Stmt stmt1 = lower(multi.getStmt1());
  Stmt stmt2 = lower(multi.getStmt2());
  return Block::make(stmt1, stmt2);
}

Stmt LowererImplDataflow::lowerSuchThat(SuchThat suchThat) {
  Stmt stmt = lower(suchThat.getStmt());
  return Block::make(stmt);
}


Expr LowererImplDataflow::lowerAccess(Access access) {
...
Expr LowererImplDataflow::lowerLiteral(Literal literal) {
...
Expr LowererImplDataflow::lowerNeg(Neg neg) {
  return ir::Neg::make(lower(neg.getA()));
}


Expr LowererImplDataflow::lowerAdd(Add add) {
...
Expr LowererImplDataflow::lowerSub(Sub sub) {
  return ir::Sub::make(lower(sub.getA()), lower(sub.getB()));
}


Expr LowererImplDataflow::lowerMul(Mul mul) {
...
Expr LowererImplDataflow::lowerDiv(Div div) {
  return ir::Div::make(lower(div.getA()), lower(div.getB()));
}


Expr LowererImplDataflow::lowerSqrt(Sqrt sqrt) {
  return ir::Sqrt::make(lower(sqrt.getA()));
}


Expr LowererImplDataflow::lowerCast(Cast cast) {
  return ir::Cast::make(lower(cast.getA()), cast.getDataType());
}


Expr LowererImplDataflow::lowerCallIntrinsic(CallIntrinsic call) {
  std::vector<Expr> args;
...
Stmt LowererImplDataflow::lower(IndexStmt stmt) {
  return visitor->lower(stmt);
}


Expr LowererImplDataflow::lower(IndexExpr expr) {
  return visitor->lower(expr);
}


bool LowererImplDataflow::generateAssembleCode() const {
  return this->assemble;
}


bool LowererImplDataflow::generateComputeCode() const {
  return this->compute;
}


Expr LowererImplDataflow::getTensorVar(TensorVar tensorVar) const {
  taco_iassert(util::contains(this->tensorVars, tensorVar)) << tensorVar;
  return this->tensorVars.at(tensorVar);
}


Expr LowererImplDataflow::getCapacityVar(Expr tensor) const {
  taco_iassert(util::contains(this->capacityVars, tensor)) << tensor;
  return this->capacityVars.at(tensor);
}


ir::Expr LowererImplDataflow::getValuesArray(TensorVar var) const
{
...
Expr LowererImplDataflow::getDimension(IndexVar indexVar) const {
  taco_iassert(util::contains(this->dimensions, indexVar)) << indexVar;
  return this->dimensions.at(indexVar);
}


std::vector<Iterator> LowererImplDataflow::getIterators(Access access) const {
...
set<Access> LowererImplDataflow::getExhaustedAccesses(MergePoint point,
...
Expr LowererImplDataflow::getReducedValueVar(Access access) const {
  return this->reducedValueVars.at(access);
}


Expr LowererImplDataflow::getCoordinateVar(IndexVar indexVar) const {
  return this->iterators.modeIterator(indexVar).getCoordVar();
}


Expr LowererImplDataflow::getCoordinateVar(Iterator iterator) const {
...
vector<Expr> LowererImplDataflow::coordinates(Iterator iterator) const {
...
Stmt LowererImplDataflow::initResultArrays(vector<Access> writes,
                                   vector<Access> reads,
                                   set<Access> reducedAccesses) {
  multimap<IndexVar, Iterator> readIterators;
  for (auto& read : reads) {
...
Stmt LowererImplDataflow::initResultArrays(IndexVar var, vector<Access> writes,
                                   vector<Access> reads,
                                   set<Access> reducedAccesses) {
  if (!generateAssembleCode()) {
    return Stmt();
...
Stmt LowererImplDataflow::resizeAndInitValues(const std::vector<Iterator>& appenders,
                                      const std::set<Access>& reducedAccesses) {
  if (!generateComputeCode()) {
    return Stmt();
  }
...
Stmt LowererImplDataflow::zeroInitValues(Expr tensor, Expr begin, Expr size, TensorVar var) {
...
Stmt LowererImplDataflow::declLocatePosVars(vector<Iterator> locators) {
  vector<Stmt> result;
...
Stmt LowererImplDataflow::reduceDuplicateCoordinates(Expr coordinate,
...
Stmt LowererImplDataflow::codeToInitializeIteratorVar(Iterator iterator, vector<Iterator> iterators, vector<Iterator> rangers, vector<Iterator> mergers, Expr coordinate, IndexVar coordinateVar) {
  vector<Stmt> result;
...
Stmt LowererImplDataflow::codeToInitializeIteratorVars(vector<Iterator> iterators, vector<Iterator> rangers, vector<Iterator> mergers, Expr coordinate, IndexVar coordinateVar) {
  vector<Stmt> results;
...
Stmt LowererImplDataflow::codeToRecoverDerivedIndexVar(IndexVar underived, IndexVar indexVar, bool emitVarDecl) {
...
Stmt LowererImplDataflow::codeToIncIteratorVars(Expr coordinate, IndexVar coordinateVar, vector<Iterator> iterators, vector<Iterator> mergers) {
  if (iterators.size() == 1) {
...
Stmt LowererImplDataflow::codeToLoadCoordinatesFromPosIterators(vector<Iterator> iterators, bool declVars) {
  // Load coordinates from position iterators
...
Stmt LowererImplDataflow::appendCoordinate(vector<Iterator> appenders, Expr coord) {
  vector<Stmt> result;
  for (auto& appender : appenders) {
...
Stmt LowererImplDataflow::generateAppendPositions(vector<Iterator> appenders) {
  vector<Stmt> result;
  if (generateAssembleCode()) {
...
Expr LowererImplDataflow::generateValueLocExpr(Access access) const {
...
Expr LowererImplDataflow::checkThatNoneAreExhausted(std::vector<Iterator> iterators)
{
  taco_iassert(!iterators.empty());
...
Expr LowererImplDataflow::generateAssembleGuard(IndexExpr expr) {
...
bool LowererImplDataflow::isAssembledByUngroupedInsertion(TensorVar result) {
  return util::contains(assembledByUngroupedInsert, result);
}


bool LowererImplDataflow::isAssembledByUngroupedInsertion(Expr result) {
...
bool LowererImplDataflow::hasStores(Stmt stmt) {
  if (!stmt.defined()) {
    return false;
  }
...
Expr LowererImplDataflow::searchForStartOfWindowPosition(Iterator iterator, ir::Expr start, ir::Expr end) {
...
Expr LowererImplDataflow::searchForEndOfWindowPosition(Iterator iterator, ir::Expr start, ir::Expr end) {
    taco_iassert(iterator.isWindowed());
    vector<Expr> args = {
            // Search over the `crd` array of the level,
            iterator.getMode().getModePack().getArray(1),
...
Stmt LowererImplDataflow::upperBoundGuardForWindowPosition(Iterator iterator, ir::Expr access) {
  taco_iassert(iterator.isWindowed());
...
Stmt LowererImplDataflow::strideBoundsGuard(Iterator iterator, ir::Expr access, bool incrementPosVar) {
...
Expr LowererImplDataflow::projectWindowedPositionToCanonicalSpace(Iterator iterator, ir::Expr expr) {
  return ir::Div::make(ir::Sub::make(expr, iterator.getWindowLowerBound()), iterator.getStride());
}


Expr LowererImplDataflow::projectCanonicalSpaceToWindowedPosition(Iterator iterator, ir::Expr expr) {
  return ir::Add::make(ir::Mul::make(expr, iterator.getStride()), iterator.getWindowLowerBound());
}

util::ScopedSet<Iterator> LowererImplDataflow::getAccessibleIterators() const {
  return accessibleIterators;
}

map<TensorVar, TemporaryArrays> LowererImplDataflow::getTemporaryArrays() const {
  return temporaryArrays;
}
void LowererImplDataflow::insertTemporaryArrays(TensorVar key, TemporaryArrays val) {
  temporaryArrays.insert({key, val});
}

ProvenanceGraph LowererImplDataflow::getProvGraph() const {
  return provGraph;
}

vector<IndexVar> LowererImplDataflow::getDefinedIndexVarsOrdered() const {
  return definedIndexVarsOrdered;
}

map<IndexVar, vector<Expr>> LowererImplDataflow::getUnderivedBounds() const {
  return underivedBounds;
}

Iterators LowererImplDataflow::getIterators() const {
  return iterators;
}

map<IndexVar, ir::Expr> LowererImplDataflow::getIndexVarToExprMap() const {
  return indexVarToExprMap;
}

ParallelUnit LowererImplDataflow::getAtomicParallelUnit() const {
  return atomicParallelUnit;
}

std::map<TensorVar, Where> LowererImplDataflow::getTempNoZeroInit() const {
