...
LinalgAssignment LinalgBase::operator=(const LinalgExpr& expr) {
  ...
  TensorVar var;
  if (isa<LinalgVarNode>(this->ptr)) {
    var = to<LinalgVarNode>(this->get())->tensorVar;
  } else if (isa<LinalgTensorBaseNode>(this->ptr)) {
    var = to<LinalgTensorBaseNode>(this->get())->tensorVar;
  } else {
...
IndexStmt LinalgBase::rewrite() {
  if (this->assignment.defined()) {
    ...
    if (tensor.getOrder() == 1) {
      ...
      if (this->isBlocked()) {
        indices.push_back(IndexVar("i"));
        indices.push_back(IndexVar("ib"));
      } else {
        indices.push_back(IndexVar("i"));
        indices.push_back(IndexVar("j"));
      }
    } else if (tensor.getOrder() == 4) {
      indices.push_back(IndexVar("i"));
      indices.push_back(IndexVar("ib"));
      indices.push_back(IndexVar("j"));
      indices.push_back(IndexVar("jb"));
    }

    Access lhs = Access(tensor, indices);
    IndexExpr rhs;
    if (isBlocked())
      rhs = blockedRewrite(this->assignment.getRhs(), indices);
    else
      rhs = rewrite(this->assignment.getRhs(), indices);
...
bool LinalgBase::isBlocked() const{
  return this->block != 0;
}
