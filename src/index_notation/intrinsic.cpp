...
Datatype PowIntrinsic::inferReturnType(const std::vector<Datatype>& argTypes,
                                        const std::vector<Datatype>& attrTypes) const {
  taco_iassert(argTypes.size() == 2 && attrTypes.empty());
  return argTypes[0];
}

ir::Expr PowIntrinsic::lower(const std::vector<ir::Expr>& args,
                             const std::vector<ir::Expr>& attrs) const {
  taco_iassert(args.size() == 2 && attrs.empty());
...
Datatype MaxIntrinsic::inferReturnType(const std::vector<Datatype>& argTypes,
                                       const std::vector<Datatype>& attrTypes) const {
  taco_iassert(argTypes.size() == 2 && attrTypes.empty());
  return argTypes[0];
}

ir::Expr MaxIntrinsic::lower(const std::vector<ir::Expr>& args,
                             const std::vector<ir::Expr>& attrs) const {
  taco_iassert(args.size() == 2 && attrs.empty());
...
Datatype HeavisideIntrinsic::inferReturnType(const std::vector<Datatype>& argTypes,
                                       const std::vector<Datatype>& attrTypes) const {
  taco_iassert(argTypes.size() == 2 && attrTypes.empty());
  return argTypes[0];
}

ir::Expr HeavisideIntrinsic::lower(const std::vector<ir::Expr>& args,
                             const std::vector<ir::Expr>& attrs) const {
  taco_iassert(args.size() == 2 && attrs.empty());
